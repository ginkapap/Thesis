...
dfclass25_1 ['累積學期pr平均pic'] =dfclass25_1.groupby(['學號'])['學期pr平均'].apply(lambda x: x.cumsum())
...
dfpic8 = dfpic8.groupby(['學號','學年','學期','年級','是否被二一'])['累積學期pr平均pic'].mean().reset_index(name='累積學期pr平均')
...
ggplot(py$dfpic11, aes(x=年級, y=該學期必修修課比例, fill=是否被二一))+ geom_boxplot( outlier.shape = NA) + ylim(0,0.8)
```
...
#pic2
dfpic99 = df.drop(['學號', '學年', '學期', '入學年','修課幾學期', '必修被當', '當學期必修修課數','選修被當','累積必修修課數', '累積必修被當', '當學期選修修課數','通識被當', '當學期通識修課數','累積通識修課數', '累積通識被當','個人外系修課數', '班平均', '班標準差', '外系標準化修課數','學期pr平均','群聚指標', '累計群聚指標', '標準化群聚指標','好友','系必修數','累積選修被當','累積選修修課數','累計標準化群聚指標','累積學期pr平均','累計外系修課指標','累積通識被當','累積選修被當','累積必修被當'],axis=1)
dfpic99.rename(columns={'累計標準化群聚指標pic':'累計標準化群聚指標','累積學期pr平均pic':'累積學期pr平均','累計外系修課指標pic':'累計外系修課指標','累積通識被當pic':'累積通識被當','累積選修被當pic':'累積選修被當','累積必修被當pic':'累積必修被當'}, inplace=True)
```
```{r results='asis',message=FALSE}
