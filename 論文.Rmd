...
原始資料為99年至106年學生各科成績分數資料，在此先排除修課學期不滿八學期者及僅使用學生有完整修過四年以上的課程作為本篇使用資料，資料中並無顯示二一紀錄，在此建立被解釋變數“是否被二一”行。
...
通識中位數算法與選修算法相同，先算出各班的必修中位數在以系分別平均。

```{python}
#計算每人通識課數、 各班中位數
dfclass3 = df[df['必選修類別（必／選／通）'] == '通']
dfclass3 = dfclass3.groupby(['系別','入學年','班別','學號']).size().reset_index(name = '個人通識數')
dfclass3 = dfclass3.groupby(['系別','入學年','班別'])['個人通識數'].median().reset_index(name = '班通識中位數')
dfclass3 = dfclass3.groupby(['系別'])['班通識中位數'].mean().reset_index(name = '通識中位數')
```
```{r results="asis"}
...
df=pd.merge(df,dfclass3_1_3,on = ['學號','學年','學期'])
```

### 累計外系修課狀況

外系的修課狀況是否也會影響到二一的預測也是我們想知道的，故加入變數之中，藉由累計外系修課數，觀察同學在於外系修課的狀況，由表X知道各系會因為規定不同，導致外系修課數有所不同，所以在此我們做標準化處理（減班平均處以班標準差）。
