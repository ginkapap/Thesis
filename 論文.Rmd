...
---------------fix--------
```{python}
#df99 = df5[(df5['系別'] == '經濟學系') & (df5['是否為必修'] == 1)]
#df98 = df99.groupby(['科目名稱','入學年']).size().reset_index()
df97 = df[(df['系別'] == '經濟學系') & (df['入學年'] == '103') &(df['必選修類別（必／選／通）']=='必') & (df['學號'] ==  410373020)]
for x in df97['科目名稱']:
  print(x)
```
```{r results="asis"}
py$df98%>% DT::datatable()
```
---------------fix
```{r results="asis"}
...
- 外系課程衡量方式：該門外系課成績與該門外系課的修課同學做pr比較，得到各外系課pr後在以同學期做平均得到不同學期的外系課pr。
