---
title: "論文"
output: 
  html_document: 
    toc: true
    toc_depth: 3
    number_sections: true
    toc_float:
      collapsed: true
      smooth_scroll: false
---


```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE,fig.showtext=TRUE, results = "hide")
library(dplyr)
library(knitr)
library(magrittr)
library(kableExtra)
library(DT)
library(stringr)
library(readr)
library(htmltools)
library(ggplot2)
library(purrr)
library(lubridate)
library(tidyr)
library(showtext)
#font_add("QYuan","cwTeXQYuan-Medium.ttf")
showtext_auto(enable=TRUE)
theme_set(theme_classic())
```
```{r}
library("reticulate")
use_condaenv("ginkapap")
#conda_install(envname = "ginkapap",c("pandas"))
#conda_install(envname = "ginkapap",c("keras",'tensorflow'))
#py_available()
```

# 目的
以往由老師發送期中預警給學生時通常學期已過一半，難以在學生遇到學習困難的初期給予協助，希望藉由二一預測系統能提早觀察出哪些學生是需要被特別關心的，藉由前幾學期的修課面向、同儕之間的關係面向、借書面向、欲預測學期的課程面向與老師面向；預測下一學期是否會被二一。

# 資料處理
## 被解釋變數
原始資料為100年至107年學生各科成績分數資料，並無顯示二一紀錄，在此建立被解釋變數“是否被二一”行。（要drop 107）

```{python}
import pandas as pd
import numpy as np
df = pd.read_csv('~/Dropbox/M-Team/research-transcript-and-student-types/main_student2.csv')
df['實拿學分'] = np.where(df['學期成績'] >= 60, df['學分數'], 0)
df2 = df.groupby(['學號','學年','學期'])['實拿學分','學分數'].sum().reset_index()
df2['是否被二一'] = np.where(df2['實拿學分']/df2['學分數'] <= 1/2 ,1 ,0)
df3 = df2[df2['是否被二一'] == 1]
df3 = df3.groupby('學號').size().reset_index(name = '二一次數')
df4 = df3.groupby('二一次數').size().reset_index(name = '人數')
```
```{r results="asis"}
py$df4 %>% kable() %>% kable_styling(full_width = F)
ggplot(py$df4 , aes(x= 二一次數,y=人數)) + geom_bar(stat="identity") + coord_flip()
```

從99學年度起至107學年度中，共有645511筆學生於不同學期的資料，其中學生被二一的紀錄有1283筆，由上表可知在被二一的同學之中被二一壹次的紀錄最多筆，共有716筆，被二一兩次的有200筆；三次以上則有44筆。



## 修課面向
### 累計被當比例
同學會被二一最直接連想的為此為同學歷年來的修課狀況與紀錄，在此新增變數分別為；累計必修被當比例、累計選修被當比例以及累計通識被當比例作為捕捉修課面向的特徵，選擇比例而非堂數的原因可由<表X>看出；各系對於必選修與通識上的要求並不一樣固使用比例。

```{python}
#刪掉所有系級年級數並保留法律系各組
df['系別'] = np.where(df['系級'].astype(str).str[0] == '法',df['系級'].str[:-1],df['系別'])
df['班別'] = df['班別'].fillna('無')
```
```{python}
#入學年
df['入學年'] = df['學號'].astype(str).str[1:4]
#計算各班人數
dfnum = df.groupby(['系別','入學年','學號','班別']).size().reset_index()
dfnum = dfnum.groupby(['系別','入學年','學號','班別']).size().reset_index(name = 'x').groupby(['系別','入學年','班別'])['x'].sum().reset_index(name = '該班人數')
df = pd.merge(df,dfnum,on=['系別','入學年','班別'],how='left')
#將體育課名統一化
df['名稱'] = np.where(df['科目名稱'].str[:2] == '體育', '體育',df['科目名稱'])
```
#計算必修中位數
```{python}
#計算各科目中各班幾人修
df5 = df[df['必選修類別（必／選／通）']=='必']
df5_1 = df5.groupby(['系別','名稱','入學年','班別']).size().reset_index(name = '該班幾人修')
df5 = pd.merge(df5,df5_1,on=['系別','名稱','入學年','班別'],how='left')
#若該班修課人數>該班人數的九成則那門課算為必修課
df5['是否為必修'] = np.where(df5['該班幾人修'] >= df5['該班人數'] *0.9 , 1 , 0)
#計算每人必修課數-> 各班中位數
dfclass1 = df5.groupby(['系別','入學年','學號'])['是否為必修'].sum().reset_index(name = '必修數').groupby(['系別','入學年'])['必修數'].median().reset_index(name = '必修數')
#去除掉98、99、進修部後計算各系中位數
dfclass1 = dfclass1[dfclass1['入學年'].str[0] != '9']
dfclass1 = dfclass1.groupby(['系別'])['必修數'].median().reset_index(name = '必修中位數')
dfclass1 = dfclass1[dfclass1['系別'].str[0] != "("]
```
```{r results="asis"}
py$dfclass1_1_1%>% DT::datatable()
```


上表可知每系的必修課開課狀況不一樣，故在此使用比例來計算。

```{python}
#整理df刪除多餘行
#df=df.drop(['Unnamed: 0','index','科目名稱','授課語言','上課時間及教室','mainstudent','mainnumber','mainQ2/3'],axis=1)
#必修不及格各學期狀況
df['不及格']=np.where(df['學期成績'] < 60 , 1 , 0)
dfclass1_1=df[df['必選修類別（必／選／通）'] == '必']
dfclass1_1_1=dfclass1_1.groupby(['學號','學年','學期'])['不及格'].sum().reset_index(name = '必修被當')
dfclass1_1_1['累積必修被當'] =dfclass1_1_1.groupby(['學號'])['必修被當'].apply(lambda x: x.cumsum())
df = pd.merge(df,dfclass1_1_1,on=['學號','學年','學期'],how='left')
```


### 累計外系修課狀況
外系的修課狀況是否也會影響到二一的預測也是我們想知道的，固加入變數之中，藉由累計外系修課數，觀察同學在於外系修課的狀況，由表X知道各系會因為規定不同，導致外系修課數有所不同，所以在此我們做標準化處理（減班平均處以班標準差）。

<表 顯示各系外系修課數>

```{python}

```


### 必修/選修/通識成績表現（pr）（怕會跟累計被當比例用共線的問題，待討論）
## 同儕面向
### 累計群聚指標
相信同學會被二一不僅僅是純粹學習上的問題，也有可能受到同儕之間群聚效應的影響，為了捕捉同學於班上的互動狀況如何，我們創造一新的變數學期(學年)群聚指標以衡量學生在於一學期(學年)的課程之中平均一週會見到多少同班同學；創造作法為藉由學生修課星期與第幾節課來尋找同班同學有多少人也有一起修課，最後藉由表X可觀察出各系的群聚指標會因為各系課程上的差異而有所差異，所以我們再將群聚指標做標準化（減去同班的平均再除以班上的標準差），作為解釋變數。

<表，做出各系的群聚指標>

### 同儕影響力
除了捕捉到班上同學們群聚效果所營造出的學習環境外，好友之間的影響力也許也是一個可以觀察的變數，我們捕捉各個同學之中與自己課表重複度最高的同學當作該同學最好的朋友，將此好友修課面向的變數也列入欲預測同學的變數中。

###欲預測學期特徵
- 欲預測學期選修數
- 欲預測學期通識數
- 曾被欲預測學期課程的老師當掉次數
- 欲預測學期課程被當過的科目數
- 是否曾經被21











