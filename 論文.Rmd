...
# 資料處理
## 被解釋變數
原始資料為100年至107年學生各科成績分數資料，並無顯示二一紀錄，在此建立被解釋變數“是否被二一”行。（到drop 107）
...
從99學年度起至107學年度中，共有645511筆學生於不同學期的資料，其中學生被二一的紀錄有1283筆，由上表可知在被二一的同學之中被二一壹次的紀錄最多筆，共有716筆，被二一兩次的有200筆；三次以上則有44筆，被二一第一次後再被二一的機率為 0.2541667。
...
df11['系級tem'] = df['開課系所'].str[:4]
a = []
for i in range(len(df11)):
    if df11['系級tem'][i] == "法律學系":
        a.append("法律學系")
    else:
        a.append(df['開課系所'][i])
df['開課系別'] = a
df = df.drop(['系級tem'], axis=1)
#排除本科系開給外科系的必修課
df5 = df[df['必選修類別（必／選／通）'] == '必']
df5 = df5[df5['開課系別'] == df5['系別']]
#將體育課名統一化
df5['名稱'] = np.where(df5['科目名稱'].str[:2] == '體育', '體育',df5['科目名稱'])
#製作中位數
df6 = df5.groupby(['系別','學年','學期','名稱']).size().reset_index()
df7 = df6.groupby(['系別','學年','學期'])['名稱'].count().reset_index(name = '必修開課數')
df8 = df7.groupby(['系別'])['必修開課數'].median().reset_index(name = '必修開課數中位數')
```
```{r results="asis"}
py$df8%>% DT::datatable()
```
